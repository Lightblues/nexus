...
Same feature surface as `../uploader.md` — clipboard paste, file drag, tray icon
drop, GitHub upload via jsDelivr. The Swift port replaces `sharp` (libvips binding)
with native ImageIO + `vImage`, and replaces the Node `Buffer` flow with Swift `Data`.
...
Replaces `sharp`. `auto` mode mirrors the Electron decision tree (`../uploader.md`):
...
- `~/.ea/nexus/uploader.json` — array of `UploadRecord` (schema in `../uploader.md`)
