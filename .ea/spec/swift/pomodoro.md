...
Same user-facing behavior as the Electron version — see `../pomodoro.md` for feature
list, state machine, and config schema. This file specs the Swift implementation.
...
Frame: 320 × 400 (matches Electron). Layout follows `../pomodoro.md` exactly:
...
Schema in `ConfigSchema.swift` matches `pomodoro.*` block in `../pomodoro.md`. The
`Config.swift` watcher fires `Config.publisher` → service rebinds durations live.
No app restart for config changes (preserves Electron hot-reload behavior).
